...
static uint32_t previous_tcxo_time;
static uint32_t previous_suspend_tcxo_time;
static int tcxo_time_overflow_counter;
static int suspend_tcxo_time_overflow_counter;
...
static void print_sleep_stat(int flag)
{
	...
	if (!sleep_stat)
		return;
	if (previous_tcxo_time > sleep_stat->tcxo_time)
		tcxo_time_overflow_counter++;
	if (previous_suspend_tcxo_time > sleep_stat->suspend_tcxo_time)
		suspend_tcxo_time_overflow_counter++;

	pr_info("sleep_stat.%d: %ds %d %ds %d %d %d %d "
		"(%d-%02d-%02d %02d:%02d:%02d.%09lu UTC)\n",
		flag, (sleep_stat->tcxo_time >> 15) + (tcxo_time_overflow_counter << 17), sleep_stat->tcxo_cnt,
		(sleep_stat->suspend_tcxo_time >> 15) + (suspend_tcxo_time_overflow_counter << 17),
	...
	previous_tcxo_time = sleep_stat->tcxo_time;
	previous_suspend_tcxo_time = sleep_stat->suspend_tcxo_time;
}
