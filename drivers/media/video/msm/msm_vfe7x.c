...
{
	int rc = 0;
	struct msm_camera_sensor_info *sinfo = dev->dev.platform_data;

	pr_info("%s sinfo->event_wait %p sinfo->waked_up %p\n", __func__, sinfo->event_wait, sinfo->waked_up);
	if (sinfo->event_wait != NULL && sinfo->waked_up != NULL) {
		pr_info("%s %d sinfo->waked_up %d\n", __func__, __LINE__, *sinfo->waked_up);
