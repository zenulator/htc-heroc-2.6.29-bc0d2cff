	      ...
	      const char *buf, size_t count)
{
	...
	case CABC_MODE:
		if (res >= CABC_OFF && res < CABC_UNDEF) {
			cabc->mode_cabc = res;
			cabc->cabc_config->change_cabcmode(
				cabc->cabc_config->change_cabcmode, res, 0x25);
		}
		break;
	case BC_MODE:
...
static void
himax_cabc_suspend(struct early_suspend *h)
{
...
static void
himax_cabc_resume(struct early_suspend *h)
{
	struct cabc *cabc = to_cabc(h, early_suspend);
